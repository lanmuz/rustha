...
pub fn main_set_common(_key: String, _value: String) {
    ...
    #[cfg(any(target_os = "windows", target_os = "macos"))]
    {
        ...
        // Drop update-related commands coming from Flutter.
        let updates_enabled = false;
        if !updates_enabled
            && (_key == "download-new-version" || _key == "update-me" || _key == "extract-update-dmg")
        {
            log::info!("Ignoring update action '{}' (updates disabled)", _key);
        } else if _key == "download-new-version" {
