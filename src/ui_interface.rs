...
pub async fn change_id_shared_(id: String, _old_id: String) -> &'static str {
    ...
    // Mobile (Android/iOS): do NOT validate/occupy the ID on rendezvous servers.
    // Only persist the user-provided ID locally.
    // This keeps Flutter's async status semantics: empty string means success.
    #[cfg(any(target_os = "android", target_os = "ios"))]
    {
