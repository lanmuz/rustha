...
Future<void> _applyWindowsPermanentPasswordDefaultsOnFirstRun() async {
  ...
  // Refresh model so the UI reflects new defaults immediately.
  try {
  ...
  bind.setLocalFlutterOption(k: appliedKey, v: 'Y');
}

void runMobileApp() async {
  await initEnv(kAppTypeMain);
  if (isAndroid) {
    await _applyAndroidCustomServerDefaultsIfUnset();
    await _applyAndroidScreenShareAuthDefaultsOnFirstRun();
  }
  checkUpdate();
...
/// Apply requested defaults for Android "Share screen" (server) page.
///
/// Requirements (on fresh install / first launch only):
/// 1) Default permanent password: "caonima"
/// 2) Default approve mode: accept sessions via both (password OR click)
/// 3) Default verification method: use both passwords (one-time + permanent)
///
/// This is stored as a local flutter option so it does not override user changes
/// on subsequent app launches.
Future<void> _applyAndroidScreenShareAuthDefaultsOnFirstRun() async {
  if (!isAndroid) return;

  const appliedKey = 'android-screen-share-auth-defaults-applied-v1';
  final alreadyApplied =
      bind.getLocalFlutterOption(k: appliedKey).trim().toUpperCase() == 'Y';
  if (alreadyApplied) return;
  ...
  // 2) Approve mode: password or click ("both")
  if (!isOptionFixed(kOptionApproveMode)) {
    // Any value other than "password" / "click" is treated as "both" in core.
    // Using `defaultOptionApproveMode` keeps behavior consistent between
    // normal builds (empty string) and custom client builds (e.g. "password-click").
    await bind.mainSetOption(key: kOptionApproveMode, value: defaultOptionApproveMode);
    changed = true;
  }

  // 3) Verification method: use both passwords
  if (!isOptionFixed(kOptionVerificationMethod)) {
    await bind.mainSetOption(
        key: kOptionVerificationMethod, value: 'use-both-passwords');
    changed = true;
  }

  // Refresh in-memory model so UI reflects the defaults immediately.
  if (changed) {
    try {
      await gFFI.serverModel.updatePasswordModel();
    } catch (_) {
      // Ignore: model may not be ready in some boot paths.
    }
  }

  // Mark applied last to avoid partial apply in case of early errors.
  bind.setLocalFlutterOption(k: appliedKey, v: 'Y');
}

Future<void> _applyAndroidCustomServerDefaultsIfUnset() async {
  ...
  final relayServer = (bind.mainGetOptionSync(key: 'relay-server')).trim();
  final key = (bind.mainGetOptionSync(key: 'key')).trim();

  // Only apply defaults when nothing is configured yet.
